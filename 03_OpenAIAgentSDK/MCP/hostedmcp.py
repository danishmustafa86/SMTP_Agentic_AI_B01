...
# Allow running from MCP/ or project root
...
from examples.automode import confirm_with_fallback
...
async def main(verbose: bool, stream: bool) -> None:
    ...
    # Use a model with higher TPM; default gpt-4.1-long-context can hit 429 on large MCP context
    agent = Agent(
        name="MCP Assistant",
        model="gpt-4o",
        instructions=(
    ...
    try:
        ...
        print(
            "Rate limit exceeded (request too large or TPM exceeded). "
            "The script uses model='gpt-4o'; try a smaller model (e.g. gpt-4o-mini) or wait and retry.",
            file=sys.stderr,
        )
        raise
